...
def setup_scrape() -> str:
    path_dst = os.path.join('itl2024_data', dt.utcnow().strftime('%Y%m%d'))
...
def scrape_charts(path_dst: str):
    ...
    for i in range(10000):
        try:
            r = requests.get(f'https://itl2024.groovestats.com/api/chart/{i}')
            j = r.json()
...
def scrape_entrants(path_dst: str):
    ...
    for i in range(10000):
        try:
            r = requests.get(f'https://itl2024.groovestats.com/api/entrant/{i}')
            j = r.json()
        except Exception as e:
            j = {'success': False, 'message': str(e)}
...
def scrape_scores(path_dst: str):
    ...
    for c in charts.values():
        ...
        for retries in range(5):
            try:
                r = requests.post(
                    f'https://itl2024.groovestats.com/api/score/chartTopScores',
